...
For now, just listing the functions for each of the objects from the PythonScript 1.0.8.0 documentation:

=head2 class Notepad

This gives access to the actual Notepad++ application.

In PythonScript, the default instance is called C<notepad>, so C<$notepad> in Perl

    .activateBufferID(bufferID)
    .activateFile(filename)
    .activateIndex(view, index)
    ...
    .closeAllButCurrent()
    .createScintilla()
    .destroyScintilla(editor)
    .getCommandLine()
    .getCurrentBufferID()
    .getCurrentDocIndex(view)
    .getCurrentFilename()
    .getCurrentLang()
    .getCurrentView()
    .getEncoding([bufferID])
    .getFiles()
    .getFormatType([bufferID])
    .getLangType([bufferID])
    .getNppDir()
    .getPluginConfigDir()
    .getPluginMenuHandle()
    .getVersion()
    ...
    .new()
    .open(filename)
    .prompt(prompt, title, defaultText)
    .reloadBuffer(bufferID)
    .reloadCurrentDocument()
    .reloadFile(filename)
    .runMenuCommand(menuName, menuOption, refreshCache)
    .runPluginCommand(pluginName, menuOption, refreshCache)
    .save()
    .saveAllFiles()
    .saveAs()
    .saveAsCopy()
    .saveCurrentSession(filename)
    .saveSession(filename, filesList)
    .setCurrentLang(langType)
    .setFormatType(formatType, bufferID)
    .setLangType(langType, bufferID)
    .setStatusBar(statusBarSection, text)
    .showTabBar()

=head2 class Editor

This gives access to the underlying Scintilla editors.
...
This gives access to the console window via C<$console>.
